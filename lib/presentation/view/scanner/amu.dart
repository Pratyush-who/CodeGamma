...
class _AmuAnalysisScreenState extends State<AmuAnalysisScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text(
          'AMU Analysis',
          style: TextStyle(color: AppColors.whiteColor, fontWeight: FontWeight.w600),
        ),
      ),
      body: isLoading
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(Icons.analytics, color: AppColors.whiteColor, size: 28),
          ),
            ...
            child: Column(
              ...
              children: [
                Text(
                  ...
                  style: const TextStyle(color: AppColors.whiteColor, fontSize: 18, fontWeight: FontWeight.bold),
                ),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Overall Assessment', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 10),
            infoRow('Compliant', assessment['compliant'].toString()),
            infoRow('Total Medications', assessment['total_medications'].toString()),
            infoRow('Total Anomalies', assessment['total_anomalies'].toString()),
            infoRow('Critical Drugs Used', assessment['critical_drugs_used'].toString()),
  ...
  Widget medicationAnalysis() {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Medication Analysis', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        const SizedBox(height: 10),
        ...meds.map((med) => Card(
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
              ...
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(med['medication'], style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    infoRow('Risk Level', med['risk_level']),
                    infoRow('Overall Compliance', med['overall_compliance'].toString()),
                    const SizedBox(height: 8),
                    Text('Anomalies:', style: const TextStyle(fontWeight: FontWeight.w600)),
                    ...((med['anomalies'] as List).map((anomaly) => Padding(
  ...
  Widget anomalyCard(Map<String, dynamic> anomaly) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(anomaly['anomaly_type'], style: const TextStyle(fontWeight: FontWeight.bold)),
          ...
          Text('Recommendation: ${anomaly['recommendation']}', style: const TextStyle(fontStyle: FontStyle.italic)),
        ],
      ),
    );
  }
      ...
      children: [
        ...
        ...recs.map((rec) => Container(
              ...
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Icon(Icons.lightbulb_outline, color: AppColors.primaryColor),
